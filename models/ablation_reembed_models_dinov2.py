...
class union_model_w_reembed(nn.Module):
    ...
    def forward(self, x_categ, x_numer, img_data, inner_slice_mask, inter_slice_mask, mode='test'):
        ...
        B = img_data.shape[1]
        img_data = img_data.squeeze(0).permute(0,3,1,2).float() # B C 16 16

        pred_mask = self.atten_pool(img_data).squeeze(1)
        gt_mask = inner_slice_mask[0,:,1:].reshape(B,16,16)
        ...
        slice_feats = torch.nn.functional.interpolate(slices_feat, size=(128,16,16), scale_factor=None, mode='trilinear', align_corners=None)
