...
class TabTransformer(nn.Module):
    ...
    def forward(self, x_categ, x_cont, return_attn = False):
        ...
        # # zyn modified add disturb
        # lamb = self.generate_random_lambda(x)
        # aug_x = self.disturb_embedding(x, lamb)
        # aug_logits = self.mlp(aug_x)
        # if not return_attn:
        #     return logits, aug_logits
        # return logits, aug_logits, attns
    ...
    def disturb_embedding(self, embedding, lamb, disturb_embedding=None): # 效果还不错 ： 4 2 0.6 | 6 3 0.6 | 6 3 0.5 # 不太好：8 4 0.6 
        ...
        disturb_embedding = disturb_embedding.where(
            lamb.bool(), torch.ones_like(disturb_embedding)
        )
        return embedding * disturb_embedding
        # return (embedding * disturb_embedding).detach() # 效果变差