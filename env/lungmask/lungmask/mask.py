...
class LMInferer:
    ...
    def extract_feat(self, image: Union[sitk.Image, np.ndarray]) -> np.ndarray:
        model = self.model
        ...
        timage_res = np.empty((np.append(0, (1024,16,16))), dtype=np.uint8)
        ...
        with torch.inference_mode():
            for mbnp in tqdm(
                ...
                feat = model.get_high_feat(mbt).cpu().detach().numpy()
                timage_res = np.vstack((timage_res, feat))

        return torch.from_numpy(timage_res)
