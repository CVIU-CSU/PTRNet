...
import argparse
import random
from models.ablation_fusion_models import union_model, union_model_slice_wise, union_model_concat, union_model_add, union_model_add_plus, union_model_gated, union_model_Film
...
def get_arguments():
    ...
    parser.add_argument("--model_mode",type=str,default='hadamard',choices=['concat','mmtm_squeeze','union','add','weighted', 'learned', 'hadamard','film','gated'])
...
if __name__ == '__main__':
    ...
    elif args.model_mode == 'concat':
        model = union_model_concat(d_model=args.d_model)
    elif args.model_mode == 'add':
        model = union_model_add(d_model=args.d_model)
    elif args.model_mode == 'mmtm_squeeze':
        model = union_model_slice_wise(d_model=args.d_model)
    elif args.model_mode == 'film':
        model = union_model_Film(d_model=args.d_model)
    elif args.model_mode == 'gated':
        model = union_model_gated(d_model=args.d_model)
    else:
        model = union_model_add_plus(method=args.model_mode)
