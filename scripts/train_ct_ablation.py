...
import argparse
import random
from models.ablation_ct_models import image_model, image_model_add_conv, image_model_add_reembed, image_model_none
...
def get_arguments():
    ...
    parser.add_argument("--model_mode",type=str,default='all',choices=['all','reembed','conv','none'])
...
def train(network: nn.Module, dataloader, dataloader_test, data_loaderTrainTest, data_loaderTest, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        if bool(epoch % 1) is False:
            ...
            if Acc + AUC*100 > best_acc + best_auc*100:
                best_auc = AUC
                best_acc = Acc
                best_sen = Sen
                best_spe = Spe
...
if __name__ == '__main__':
    ...
    if args.model_mode == 'all':
        model = image_model(d_model=args.d_model,hidden_dim=args.hidden_dim)
    elif args.model_mode == 'reembed':
        model = image_model_add_reembed(d_model=args.d_model,hidden_dim=args.hidden_dim)
    elif args.model_mode == 'conv':
        model = image_model_add_conv(d_model=args.d_model,hidden_dim=args.hidden_dim)
    else:
        model = image_model_none(d_model=args.d_model,hidden_dim=args.hidden_dim)
