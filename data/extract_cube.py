...
file_out_path = '/home/zhangyinan/2024_05/extracted_cubes'
...
with torch.no_grad():
    for folder_name in os.listdir(dataset_path):
        ...
        nii_data = torch.from_numpy(nii_data).cuda()
        if debug:
            print(nii_data.shape)
        now_cube = nii_data[y1:y2,x1:x2,l:r+1]
        now_cube = now_cube.permute(2,0,1).unsqueeze(0).unsqueeze(0) # 1 C N H W
        now_cube = torch.nn.functional.interpolate(now_cube, size=(128,224,224), scale_factor=None, mode='trilinear', align_corners=None)
        
        if debug:
            print(now_cube.shape)
            cv2.imwrite(os.path.join(file_out_path, 'debug.png'), now_cube[0,0,100,:,:].cpu().numpy())
            exit(0)
        
        now_cube = (now_cube/255).float()
        torch.save(now_cube.squeeze(0).cpu(), os.path.join(file_out_path, folder_name+'.pt'))
