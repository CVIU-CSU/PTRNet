...
from dinov2_transforms import make_classification_eval_transform


model = torch.hub.load(repo_or_dir="/home/zhangyinan/.cache/torch/hub/facebookresearch_dinov2_main", model='dinov2_vitb14', source='local')
...
file_out_path = '/home/zhangyinan/2024_05/dinov2_features'
...
with torch.no_grad():
    for folder_name in os.listdir(dataset_path):
        ...
        for i in range(segmentation.shape[0]):
            ...
            img = Image.fromarray(process_data).convert('RGB')

            features_dict = model.forward_features(transforms(img).to(device).unsqueeze(0))
            image_features = features_dict['x_norm_patchtokens']
            N, D = image_features.shape[0], image_features.shape[2]
            image_features = image_features.reshape(N, 16, 16, D)
            ...
            save_data = [1.0]
            for j in range(0,mask_tensor.shape[0],14):
                for k in range(0,mask_tensor.shape[1],14):
                    if torch.sum(mask_tensor[j:j+14,k:k+14]) > 0:
